...
def index():
    ...
    posts = current_user.followed_posts().paginate(page, page_size, False)
...
def explore():
    page = request.args.get("page", 1, type=int)
    posts = Post.query.order_by(Post.timestamp.desc()).paginate(page, page_size, False)
...
def user(username):
    ...
    page = request.args.get('page', 1, type=int)
    posts = user.posts.order_by(Post.timestamp.desc()).paginate(page, page_size, False)
